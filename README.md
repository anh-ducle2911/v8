...
> Không có module "phân cụm" độc lập. Việc gom đơn cùng tuyến chỉ là một bước
> hỗ trợ bên trong thuật toán.

---

## 2b. Engine định tuyến HYBRID (Greedy seed → OR-Tools GUIDED_LOCAL_SEARCH)

Engine lập tuyến chạy **1 nút duy nhất** ("Chạy kế hoạch tuyến") nhưng bên trong là quy
trình lai (hybrid) tái sử dụng OR-Tools sẵn có:

- **Greedy (constructive heuristic)** — `engine_ext.solve_preset()`: dựng nhanh nghiệm ban
  đầu, gán đơn vào xe theo trọng số preset, tôn trọng đầy đủ ràng buộc (loại xe, tải, dung
  tích, cấm tải theo khung giờ). Đây là **seed**.
- **OR-Tools VRPTW** — `hpl_engine._solve_ortools()`: mô hình pickup-delivery cùng xe, cửa sổ
  thời gian, tải/dung tích, tương thích loại xe & cấm tải, cho phép bỏ đơn có phạt.
- **Metaheuristic GUIDED_LOCAL_SEARCH (GLS)** — chiến lược tìm kiếm cục bộ có hướng dẫn của
  OR-Tools, giúp thoát cực tiểu cục bộ và cải thiện nghiệm trong giới hạn thời gian.
- **Hybrid** — nạp nghiệm Greedy làm seed cho OR-Tools (`routing.ReadAssignmentFromRoutes` →
  `SolveFromAssignmentWithParameters`) rồi để GLS **cải thiện**: hội tụ nhanh hơn, nghiệm tốt
  hơn. Orchestrate tại `engine_ext.solve_hybrid()`.
- **Fallback an toàn** — OR-Tools lỗi / timeout / kém hơn / không cài đặt → tự dùng nghiệm
  Greedy. **Engine luôn trả ra output.**

Cấu hình (qua `.env`, đều có mặc định an toàn):
- `HPL_SOLVER_MODE=hybrid|ortools|greedy` (mặc định `hybrid`).
- `HPL_SOLVER_TIME_LIMIT_SEC=30` — giới hạn metaheuristic (khuyến nghị OR-Tools **60–120s**).
- Tối ưu lại tự động khi có đơn mới: bật/tắt bằng nút trên thanh Engine hoặc
  `POST /api/config {"auto_optimize": true|false}`. Tuyến `locked/in_progress/completed`
  không bị tối ưu lại.

> Chi tiết kỹ thuật, danh sách file đã sửa/thêm, cách test và bật/tắt: xem
> [`BAO_CAO_NANG_CAP.md`](BAO_CAO_NANG_CAP.md).
