...
# ============================================================
# CẤU HÌNH ENGINE ĐỊNH TUYẾN HYBRID (Greedy seed → OR-Tools improve)
# ============================================================
# 3 chế độ vẫn giữ ở backend để debug/chạy thử; MẶC ĐỊNH dùng "hybrid".
#   • "greedy"  — chỉ chạy heuristic constructive (Greedy-Insertion).
#   • "ortools" — chỉ chạy OR-Tools (PARALLEL_CHEAPEST_INSERTION + GLS), không seed.
#   • "hybrid"  — Greedy sinh nghiệm ban đầu (seed) → OR-Tools cải thiện bằng
#                 GUIDED_LOCAL_SEARCH. Lỗi/timeout/không cải thiện → fallback Greedy.
SOLVER_MODE = (os.environ.get("HPL_SOLVER_MODE") or "hybrid").strip().lower()
# Giới hạn thời gian metaheuristic (giây). Mặc định 20s: với quy mô bài toán hiện tại
# (≤60 đơn / 40 xe) OR-Tools đã hội tụ trong ~20s và cho biên 20–30% ổn định; nhanh hơn
# 30s mà không mất chất lượng. Có thể chỉnh qua env HPL_SOLVER_TIME_LIMIT_SEC hoặc tham số.
SOLVER_TIME_LIMIT_SEC = int(float(os.environ.get("HPL_SOLVER_TIME_LIMIT_SEC") or 20))
# Seed cố định để các lần chạy lặp lại cho kết quả ổn định (deterministic).
SOLVER_RANDOM_SEED = int(float(os.environ.get("HPL_SOLVER_SEED") or 2026))
...
# ------------------------------------------------------------
# ĐỌC FILE ĐƠN BỔ SUNG / BACKHAUL (Module 4) — 3 trường hợp:
#   1) Nhập tay  2) Import từ 3PL  3) Gợi ý tự động
# Tự nhận diện cột theo bí danh, chấp nhận khung giờ dạng "08:00-17:00".
# ------------------------------------------------------------
BACKHAUL_ALIASES = {
    "order_id": ["mã đơn bổ sung", "mã đơn 3pl", "mã gợi ý", "mã đơn", "order_id", "order id"],
    "customer": ["khách hàng", "đối tác 3pl", "nguồn dữ liệu", "customer", "khach hang"],
    "pickup_name": ["điểm lấy đề xuất", "điểm lấy", "pickup", "diem lay"],
    "delivery_name": ["điểm trả đề xuất", "điểm trả", "điểm giao", "delivery", "diem tra"],
    "pickup_lat": ["vĩ độ lấy", "lat lấy", "pickup_lat", "pickup lat"],
    "pickup_lon": ["kinh độ lấy", "lon lấy", "pickup_lon", "pickup lon"],
    "delivery_lat": ["vĩ độ trả", "lat trả", "delivery_lat", "delivery lat"],
    "delivery_lon": ["kinh độ trả", "lon trả", "delivery_lon", "delivery lon"],
    "weight_kg": ["tải trọng (kg)", "tải trọng", "khối lượng", "weight"],
    "volume_m3": ["thể tích (m³)", "thể tích", "volume"],
    "min_vehicle": ["loại xe yêu cầu", "loại xe", "min_vehicle", "vehicle"],
    "revenue": ["doanh thu dự kiến (đ)", "giá nhận đơn (đ)", "doanh thu bổ sung (đ)",
                "doanh thu", "giá nhận đơn", "revenue"],
    "tw": ["khung giờ trả", "khung giờ giao", "drop tw"],
    "inner_city": ["nội đô", "cấm tải", "inner city"],
}


def _alias_lookup(header_norm):
    """Trả về map {tên_cột_gốc: field_chuẩn} cho 1 sheet."""
    out = {}
    for raw, norm in header_norm.items():
        for field, aliases in BACKHAUL_ALIASES.items():
...
def _split_tw(v):
    """'08:00-17:00' -> (start_min, end_min). Chấp nhận '08:00 - 17:00'."""
    s = str(v or "").replace("–", "-").replace("đến", "-")
    if "-" in s:
        a, b = s.split("-", 1)
        return time_to_min(a.strip(), 0), time_to_min(b.strip(), 1440)
    return 0, 1440


def parse_backhaul_workbook(file_bytes):
    """Đọc file Excel đơn bổ sung Module 4 (mọi sheet trừ 'Huong_dan'/'Hướng dẫn').
    Trả về list đơn backhaul đã chuẩn hóa, sẵn sàng đưa vào STORE."""
    if not HAS_OPENPYXL:
        raise RuntimeError("Cần cài openpyxl: pip install openpyxl")
    wb = openpyxl.load_workbook(io.BytesIO(file_bytes), data_only=True)
    orders = []
    for sheet in wb.sheetnames:
        if sheet.strip().lower() in ("huong_dan", "hướng dẫn", "huong dan", "guide"):
            continue
        ws = wb[sheet]
        rows = list(ws.iter_rows(values_only=True))
        if not rows:
            continue
        # Tìm DÒNG TIÊU ĐỀ (có cột vĩ độ lấy & vĩ độ trả) — bỏ qua dòng banner/tiêu đề trên.
        hdr_idx, headers, amap = None, None, None
        for ri, row in enumerate(rows):
            cells = [str(h).strip() if h is not None else "" for h in row]
            hn = {h: h.lower().strip() for h in cells if h}
            am = _alias_lookup(hn)
            if "pickup_lat" in am.values() and "delivery_lat" in am.values():
                hdr_idx, headers, amap = ri, cells, am
                break
        if hdr_idx is None:
            continue  # sheet không phải dữ liệu đơn backhaul
        for r in rows[hdr_idx + 1:]:
            if r is None or all(c is None or c == "" for c in r):
                continue
            rec = {headers[i]: r[i] for i in range(min(len(headers), len(r)))}
            o = {"source": f"Excel Module 4 · {sheet}"}
            for col, field in amap.items():
                o[field] = rec.get(col)
            if not (o.get("pickup_lat") and o.get("delivery_lat")):
                continue
            tw_start, tw_end = _split_tw(o.pop("tw", None))
            orders.append({
                "order_id": o.get("order_id") or f"BK{len(orders)+1}",
                "customer": o.get("customer"),
                "pickup_name": o.get("pickup_name"), "delivery_name": o.get("delivery_name"),
                "pickup_lat": _f(o.get("pickup_lat")), "pickup_lon": _f(o.get("pickup_lon")),
                "delivery_lat": _f(o.get("delivery_lat")), "delivery_lon": _f(o.get("delivery_lon")),
                "weight_kg": _f(o.get("weight_kg")), "volume_m3": _f(o.get("volume_m3")),
                "min_vehicle": o.get("min_vehicle"), "revenue": _f(o.get("revenue")),
                "inner_city": _yes(o.get("inner_city")),
                "drop_tw_start": tw_start, "drop_tw_end": tw_end,
                "source": o["source"],
            })
    return orders
...
def solve_vrptw(orders, fleet, depots=None, respect_ban=True, mode=None,
                time_limit=None, seed_routes=None):
    """HYBRID ROUTING ENGINE — gán đơn cho xe + dựng thứ tự điểm dừng tôn trọng:
    cùng xe lấy/giao, lấy trước giao sau, tải trọng, dung tích, khung giờ, tương
    thích loại xe, cấm tải nội đô theo khung giờ.

    mode:
      • "greedy"  — chỉ heuristic constructive.
      • "ortools" — chỉ OR-Tools (PARALLEL_CHEAPEST_INSERTION + GLS).
      • "hybrid"  — Greedy seed → OR-Tools cải thiện (GUIDED_LOCAL_SEARCH);
                    lỗi/timeout/không cải thiện → fallback Greedy.

    LUÔN trả ra output (không bao giờ rơi vào trạng thái "không có kết quả").
    Trả về {status, engine, mode, routes, unassigned, fallback, time_limit_sec, improved}.
    """
    mode = (mode or SOLVER_MODE or "hybrid").strip().lower()
    if mode not in ("greedy", "ortools", "hybrid"):
        mode = "hybrid"
    tl = int(time_limit or SOLVER_TIME_LIMIT_SEC)
    ...
    if not valid or not fleet:
        return {"status": "error", "message": "Không đủ dữ liệu khả thi để chạy VRPTW.",
                "engine": "—", "mode": mode, "routes": [], "unassigned": list(valid),
                "fallback": None, "time_limit_sec": tl, "improved": False}

    # --- Nghiệm Greedy (constructive seed) — luôn tính để làm seed & làm phương án dự phòng ---
    greedy_routes, greedy_un = _solve_greedy(valid, fleet, depots, respect_ban)

    if mode == "greedy":
        return {"status": "ok", "engine": "Greedy-Insertion (constructive)", "mode": "greedy",
                "routes": greedy_routes, "unassigned": greedy_un, "fallback": None,
                "time_limit_sec": tl, "improved": False}

    if not _has_ortools():
        return {"status": "ok", "engine": "Greedy-Insertion (OR-Tools không khả dụng)",
                "mode": mode, "routes": greedy_routes, "unassigned": greedy_un,
                "fallback": "ortools_unavailable", "time_limit_sec": tl, "improved": False}

    seed = seed_routes if (seed_routes is not None and mode == "hybrid") else \
        (greedy_routes if mode == "hybrid" else None)
    try:
        ...
        if routes is not None and routes:
            # So sánh với seed greedy: nếu OR-Tools gán được ÍT đơn hơn rõ rệt -> giữ greedy.
            if _n_assigned(routes) + 0 < _n_assigned(greedy_routes):
                return {"status": "ok", "mode": mode,
                        "engine": "Greedy-Insertion (OR-Tools kém hơn → fallback)",
                        "routes": greedy_routes, "unassigned": greedy_un,
                        "fallback": "ortools_worse", "time_limit_sec": tl, "improved": False}
            eng_name = ("Hybrid: Greedy seed → OR-Tools GUIDED_LOCAL_SEARCH"
                        if mode == "hybrid" else "OR-Tools (GUIDED_LOCAL_SEARCH)")
            return {"status": "ok", "engine": eng_name, "mode": mode,
                    "routes": routes, "unassigned": unassigned, "fallback": None,
                    "time_limit_sec": tl, "improved": True}
    except Exception as e:
        return {"status": "ok", "mode": mode,
                "engine": "Greedy-Insertion (OR-Tools lỗi → fallback)",
                "routes": greedy_routes, "unassigned": greedy_un,
                "fallback": f"ortools_error: {e}", "time_limit_sec": tl, "improved": False}
    # OR-Tools trả rỗng -> fallback greedy
    return {"status": "ok", "engine": "Greedy-Insertion (OR-Tools rỗng → fallback)",
            "mode": mode, "routes": greedy_routes, "unassigned": greedy_un,
            "fallback": "ortools_empty", "time_limit_sec": tl, "improved": False}


def _n_assigned(routes):
    return sum(int(r.get("n_orders", 0)) for r in (routes or []))
...
def _seed_seq_tw_feasible(seq, tmat, tw):
    """Kiểm tra 1 chuỗi node của 1 xe có khả thi khung giờ trong mô hình Time của
    OR-Tools không (xuất phát từ node depot ảo 0, được phép CHỜ). Vì chờ được phép
    còn xuất phát sớm là tối ưu, ràng buộc chặn là CẬN TRÊN: nếu tới điểm nào trễ
    hơn cận trên -> không khả thi -> không seed xe đó (để OR-Tools tự xếp lại)."""
    cur, prev = 0, 0
    for node in seq:
        cur += tmat[prev][node]
        e, l = tw[node]
...
def _solve_ortools(orders, fleet, depots=None, respect_ban=True,
                   time_limit=None, seed_routes=None, seed_info=None):
    from ortools.constraint_solver import routing_enums_pb2, pywrapcp
    avail = _avail_fleet(fleet)
    if not avail:
        return None, []
    time_limit = int(time_limit or SOLVER_TIME_LIMIT_SEC)
    oid_to_oi = {o["order_id"]: oi for oi, o in enumerate(orders)}
    ...
    for oi, o in enumerate(orders):
        ...
        for vi, veh in enumerate(avail):
            if _compat(o, veh, respect_ban):
                allowed.append(vi)
        # SỬA LỖI: kể cả khi KHÔNG có xe nào hợp lệ (allowed rỗng) vẫn phải giới hạn
        # -> SetValues([-1]) ép đơn CHỈ có thể "không phục vụ" (vào disjunction) thay vì
        # để OR-Tools gán bừa lên xe vi phạm cấm tải/loại xe/xe lạnh.
        if len(allowed) < len(avail):
    ...
    sp.time_limit.seconds = max(1, int(time_limit))
    sp.log_search = False  # ổn định, không in log

    # --- SEED từ nghiệm Greedy (Hybrid): nạp nghiệm ban đầu để OR-Tools cải thiện ---
    # ReadAssignmentFromRoutes là "all-or-nothing": chỉ 1 xe có chuỗi node không khả
    # thi khung giờ là cả seed bị loại. Vì greedy gom đơn KHÔNG kiểm tra khung giờ,
    # ta CHỈ seed những xe có chuỗi khả thi (xe còn lại để OR-Tools tự xếp). Nhờ vậy
    # seed greedy thực sự được áp dụng (warm start) thay vì luôn cold-start.
    initial = None
    n_seeded = 0
    if seed_routes:
        try:
            init_routes = [[] for _ in range(len(avail))]
            vid_to_vi = {v["vehicle_id"]: vi for vi, v in enumerate(avail)}
            for r in seed_routes:
                vi = vid_to_vi.get(r.get("vehicle_id"))
                if vi is None:
                    continue
                objs = r.get("order_objs") or []
                pk = [1 + oid_to_oi[o["order_id"]] * 2 for o in objs if o.get("order_id") in oid_to_oi]
                dl = [1 + oid_to_oi[o["order_id"]] * 2 + 1 for o in objs if o.get("order_id") in oid_to_oi]
                seq = pk + dl  # lấy hết rồi giao hết (pickup luôn trước delivery)
                if seq and _seed_seq_tw_feasible(seq, tmat, tw):
                    init_routes[vi] = seq
                    n_seeded += 1
            if any(init_routes):
    ...
    if seed_info is not None:
        seed_info["seeded"] = initial is not None
        seed_info["n_seed_vehicles"] = n_seeded if initial is not None else 0

    if initial is not None:
        sol = routing.SolveFromAssignmentWithParameters(initial, sp)
    else:
    ...
    for vid in range(len(avail)):
        ...
        for kind, o in seq:
            stops.append({"type": kind, "order_id": o["order_id"],
                          ...
                          # SỬA LỖI: gắn khung giờ 'tw' đồng nhất với _assemble_route để route_timeline
                          # tính ETA có CHỜ khung giờ đúng (trước đây tuyến OR-Tools thiếu 'tw').
                          "tw": [o["pickup_tw_start"], o["pickup_tw_end"]] if kind == "pickup"
                                else [o["drop_tw_start"], o["drop_tw_end"]],
...
def _build_route(veh, orders, stops):
    ...
    # Quãng RỖNG (deadhead) = chiều giao cuối -> DEPOT QUAY VỀ = stops[-1] (do _nearest_depot
    # chọn), KHÔNG phải điểm xuất phát stops[0]. Đo về stops[-1] mới khớp đúng leg rỗng cuối
    # đã nằm trong full_loop -> productive_km = full_loop - empty_km luôn nhất quán, không âm.
    depot = stops[-1]
    empty_km = 0.0
    if last_delivery:
...
def compute_route_pnl(route, fin=None):
    ...
    return {
        ...
        "profit": round(profit), "margin": round(margin, 2),
        "warnings": warnings,
    }
...
def compute_financials(routes, fleet, fin=None):
    ...
    totals["margin"] = round(totals["profit"] / totals["revenue_total"] * 100, 2) if totals["revenue_total"] else 0
